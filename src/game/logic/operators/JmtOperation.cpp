#include "JmtOperation.h"

void JmtOperation::execute(Nano *currentNano, Lexer &lexer, uint8_t currentLine) {
	Token operand1 = lexer.next();

	if (operand1.type == Token::TOKEN_SYMBOL) {
		jmt(currentNano, operand1, currentLine);
	} else {
...
void JmtOperation::jmt(Nano *currentNano, Token operand1, uint8_t currentLine) {
	uint8_t labelLine = currentNano->code->getLineOfLabel(operand1.text);

	if(currentNano->test <= 0) {
		return;
	}
